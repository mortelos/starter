...
| **Operate mode** (UteqOS MCP) | Runtime agent operates the live workspace: search entities, run skills, trigger workflows, approve proposals, launch sub-agents | OAuth-authenticated MCP tools served from the host |
...
The server is provided by `uteq/mortel` (not by `mortelos/starter`). The host
mounts it:
...
Exact tool names and shapes live in `uteq/mortel` source. When implementing a
new agent capability, check there first; if you need to expose a new tool,
that's a `uteq/mortel` PR, not a starter PR.
...
- Don't expose new MCP tools from inside `mortelos/starter` — starter is the
  shell, not the agent surface. New tools go in `uteq/mortel` or a feature
  package.
...
Read `uteq/mortel/src/MCP/` (in `/Users/uteq/Sites/mortel/` or
`vendor/uteq/mortel/`) for the actual tool list. Don't guess based on this
file; the source is authoritative.
