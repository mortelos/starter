...
`routes/web.php`, host policies); those files live outside the starter repo, so the
skill must run from the host-app working directory. Because the starter is installed
as `vendor/mortelos/starter`, symlink it into the host once:
...
- **Stack detection.** Confirm this is a MortelOS host app (Laravel + Livewire 4
  + `mortelos/starter`). If `uteq-tall-master` skills are present, you will use
